...
export const STORAGE_KEY = 'think_green_platform_v6_data';
...
export const loadInitialState = (): AppState => {
  try {
    ...
    if (saved) {
      ...
      // Deduplicate all arrays to eliminate duplicate key errors from rapid saves or faulty migrations
      return {
        currentUser: parsed.currentUser || INITIAL_USERS[0],
        users: deduplicateById(parsed.users || INITIAL_USERS),
        students: deduplicateById(parsed.students || INITIAL_STUDENTS),
        courses: deduplicateById(parsed.courses || INITIAL_COURSES),
        classes: deduplicateById(parsed.classes || INITIAL_CLASSES),
        enrollments: deduplicateById(parsed.enrollments || INITIAL_ENROLLMENTS),
        charges: deduplicateById(parsed.charges || INITIAL_CHARGES),
        receipts: deduplicateById(parsed.receipts || INITIAL_RECEIPTS),
        cashRegisters: deduplicateById(parsed.cashRegisters || INITIAL_CASH_REGISTERS),
        cashMovements: deduplicateById(parsed.cashMovements || INITIAL_CASH_MOVEMENTS),
        assessments: deduplicateById(parsed.assessments || INITIAL_ASSESSMENTS),
        grades: deduplicateById(parsed.grades || INITIAL_GRADES),
        lessons: deduplicateById(parsed.lessons || INITIAL_LESSONS),
        assignments: deduplicateById(parsed.assignments || INITIAL_ASSIGNMENTS),
        submissions: deduplicateById(parsed.submissions || INITIAL_SUBMISSIONS),
        attendance: deduplicateById(parsed.attendance || INITIAL_ATTENDANCE),
        accountingTransactions: deduplicateById(parsed.accountingTransactions || INITIAL_ACCOUNTING_TRANSACTIONS),
  ...
  return {
    ...
    users: deduplicateById(INITIAL_USERS),
    students: deduplicateById(INITIAL_STUDENTS),
    courses: deduplicateById(INITIAL_COURSES),
    classes: deduplicateById(INITIAL_CLASSES),
    enrollments: deduplicateById(INITIAL_ENROLLMENTS),
    charges: deduplicateById(INITIAL_CHARGES),
    receipts: deduplicateById(INITIAL_RECEIPTS),
    cashRegisters: deduplicateById(INITIAL_CASH_REGISTERS),
    cashMovements: deduplicateById(INITIAL_CASH_MOVEMENTS),
    assessments: deduplicateById(INITIAL_ASSESSMENTS),
    grades: deduplicateById(INITIAL_GRADES),
    lessons: deduplicateById(INITIAL_LESSONS),
    assignments: deduplicateById(INITIAL_ASSIGNMENTS),
    submissions: deduplicateById(INITIAL_SUBMISSIONS),
    attendance: deduplicateById(INITIAL_ATTENDANCE),
    accountingTransactions: deduplicateById(INITIAL_ACCOUNTING_TRANSACTIONS),
    auditLogs: deduplicateById(INITIAL_AUDIT_LOGS),
    notifications: deduplicateById(INITIAL_NOTIFICATIONS),
