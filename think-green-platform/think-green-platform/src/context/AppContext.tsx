...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createStudentWithEnrollment = (
    ...
    const internalId = `stu_${Date.now()}`;
    ...
    setState((prev) => ({
      ...
      users: [newStudentUser, ...prev.users],
  ...
  const registerPayment = (
    ...
    setState((prev) => ({
      ...
      accountingTransactions: [newAccountingTx, ...prev.accountingTransactions],
    }));
  ...
  const createAccountingTransaction = (data: Omit<AccountingTransaction, 'id' | 'registeredBy' | 'status'>) => {
    ...
    setState((prev) => ({
      ...prev,
      accountingTransactions: [newTx, ...prev.accountingTransactions],
    }));
  ...
  const importStudentsBatch = (rows: any[]): number => {
    ...
    rows.forEach((row) => {
      if (!row.name) return;
      const nextStuId = generateNextStudentId(currentStudents);
      const internalId = `stu_imp_${Date.now()}_${count}`;
      const nowStr = new Date().toISOString().split('T')[0];
      ...
      const student: Student = {
        ...
        studentId: nextStuId,
        name: row.name,
        email: row.email || `${nextStuId.toLowerCase()}@student.thinkgreen.org`,
        phone: row.phone || '+20 10 0000 0000',
        city: row.city || 'Cairo',
        status: 'ATIVO',
        notes: `Importado de planilha em ${nowStr}. Dados originais: ${row.originalNotes || ''}`,
        createdAt: nowStr,
        documents: [],
      };
      ...
      const enrollment: Enrollment = {
        ...
        paymentPlan: {
          installmentsCount: 4,
          totalAmount: course.monthlyFee * 4,
          installmentAmount: course.monthlyFee,
        },
      };
    ...
    logAudit('ADMIN', 'IMPORTACAO_PLANILHA', 'BatchImport', `${count}_alunos`, undefined, `${count} alunos importados com STU IDs gerados.`);
    return count;
  };
